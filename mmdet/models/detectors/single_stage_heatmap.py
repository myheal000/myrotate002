...
def draw_feature_map1(features, img_path, save_dir='/kaggle/working/feature_map/', name=None):
    '''
    ...
    for featuremap in features:     # 循环遍历
        heatmap = featuremap_2_heatmap1(
            featuremap)  # 主要是这个，就是取特征层整个的求和然后平均，归一化
        # 将热力图的大小调整为与原始图像相同
        heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
        # 将热力图转换为RGB格式,0-255,heatmap0显示红色为关注区域，如果用heatmap则蓝色是关注区域
        heatmap0 = np.uint8(255 * heatmap)
        ...
        # superimposed_img：保存的是叠加在原图上的图，也可以保存过程中其他的自己看看
        cv2.imwrite(os.path.join(save_dir, name +
                    str(i) + '.png'), superimposed_img)
...
def feature_map_channel(features, img_path, save_dir='work_dirs/feature_map', name='noresbnsie2ltft_'):
    ...
    a = torch.squeeze(features[0][:1, :, :, :], dim=0).permute(
        1, 2, 0).detach().cpu().numpy()
    b = torch.squeeze(features[1][:1, :, :, :], dim=0).permute(
        1, 2, 0).detach().cpu().numpy()
    c = torch.squeeze(features[2][:1, :, :, :], dim=0).permute(
        1, 2, 0).detach().cpu().numpy()
    d = torch.squeeze(features[3][:1, :, :, :], dim=0).permute(
        1, 2, 0).detach().cpu().numpy()
    ...
    for j, x in enumerate([d]):
        ...
        for i in range(x.shape[-1]):
            ...
            # plt.close()
            cv2.imwrite(os.path.join(save_dir, name + str(j) +
                        str(i) + '.png'), superimposed_img)
